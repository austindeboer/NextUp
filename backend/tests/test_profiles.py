...
class TestProfilesRoutes:
    """
    Make sure no api route returns 404
    """
...
class TestProfileCreate:
    async def test_profile_created_for_new_users(self, app: FastAPI, client: AsyncClient, db: Database) -> None:
        ...
        new_user = {"email": "mia@wallace.com", "username": "miawallace123", "password": "iwantthattrophy"}
...
class TestProfileView:
    async def test_authenticated_user_can_view_other_users_profile(self, app: FastAPI, authorized_client: AsyncClient, test_user: UserInDB, test_user2: UserInDB) -> None:
    ...
    async def test_unregistered_users_cannot_access_other_users_profile(self, app: FastAPI, client: AsyncClient, test_user2: UserInDB) -> None:
        res = await client.get(app.url_path_for("profiles:get-profile-by-username", username=test_user2.username))
        assert res.status_code == status.HTTP_401_UNAUTHORIZED

    async def test_no_profile_is_returned_when_username_matches_no_user(self, app: FastAPI, authorized_client: AsyncClient) -> None:
        res = await authorized_client.get(
...
class TestProfileManagement:
    @pytest.mark.parametrize(
        ...
        (
            ...
            ("phone_number", "555-123-4455"),
            ("bio", "Testing bio"),
            ("image", "http://testimages.com/testimage"),
        ),
    )
    async def test_user_can_update_own_profile(self, app: FastAPI, authorized_client: AsyncClient, test_user: UserInDB, attr: str, value: str,) -> None:
    ...
    @pytest.mark.parametrize(
        ...
        (
            ("full_name", [], 422),
            ("phone_number", "5555-2233-42342424", 422),
